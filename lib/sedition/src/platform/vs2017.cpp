...
{
	struct msvs_solution *msvs_sols = NULL;
	sed_array_create(msvs_sols, sed_array_size(sols));

	struct msvs_project *msvs_projs = NULL;
	sed_array_create(msvs_projs, sed_array_size(projs));

	struct msvs_config *msvs_configs = NULL;
	sed_array_create(msvs_configs, sed_array_size(confs));

	/* generate windows specific information */
	{
		int i;


		for (i = 0; i < sed_array_size(sols); ++i)
		{
			
		}

		for (i = 0; i < sed_array_size(projs); ++i)
		{

		}

		for (i = 0; i < sed_array_size(confs); ++i)
		{
