...
#include <fiber/optio_dispatcher.h>
#include <stdio.h>
#include <pthread.h>
#include <string.h>
#include <assert.h>
...
void some_other_jawb(struct optio_dispatcher_ctx *fibers, void *arg)
{
  int *int_arg = (int*)arg;
  ++(*int_arg);
...
void some_jawb(struct optio_dispatcher_ctx *fibers, void *arg)
{
  ...
  {
    struct optio_job_desc job_desc[job_spawn_count_nested];
    int k;
    ...
    int job_marker = optio_dispatcher_add_jobs(fibers, job_desc, job_spawn_count_nested);
    
    optio_dispatcher_wait_for_counter(fibers, job_marker);
...
main()
{
  ...
  for(i = 0; i < job_iterations; ++i)
  {
    ...
    /* run test */
    {
      struct optio_job_desc job_desc[job_spawn_count_inital];
      int k;
      ...
      optio_dispatcher_add_jobs(fibers, job_desc, job_spawn_count_inital);
