  ...
  struct roa_renderer_task **tasks)
{
        if (roa_atomic_int_load(&ctx->tasks.curr_task_count) <= 0) {
                ...
                /* flip task buffer */
                struct roa_renderer_tasks *temp_curr = ctx->tasks.curr_tasks;

                ctx->tasks.curr_tasks = ctx->tasks.back_tasks;
                ctx->tasks.back_tasks = temp_curr;

                roa_array_clear(ctx->tasks.back_tasks);
                ...
                *tasks = ctx->tasks.curr_tasks;
                ...
                roa_spin_lock_release(&ctx->tasks.lock, task_count);
        }
